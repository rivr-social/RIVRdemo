...
import { mockJobShifts, type JobShift } from "@/lib/mock-job-shift-data"
...
export default function JobPage() {
  ...
  const [parentProject, setParentProject] = useState<any | null>(null)
  const [activeTab, setActiveTab] = useState("about")

  useEffect(() => {
    // Find the job
    const foundJob = mockJobShifts.find((j) => j.id === jobId)
    if (foundJob) {
      setJob(foundJob)
      
      // Find the parent project for this job
    ...
    <div className="container max-w-6xl mx-auto p-4 pb-20">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="tasks" className="mt-6">
          <JobTasksTab job={job} currentUserId={currentUserId} onTaskUpdate={(updatedJob) => setJob(updatedJob)} />
        </TabsContent>
