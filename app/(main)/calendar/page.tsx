...
export default function CalendarPage() {
  ...
  const shiftEvents: CalendarItem[] = userShifts.map((shift) => {
  ...
  const userEvents: (CalendarItem | null)[] = projects
    .filter((p) => p.type === "event")
    .map((event) => {
      // Handle both timeframe.start and startDate formats
      const startDate = event.timeframe?.start || new Date().toISOString()
      const endDate = event.timeframe?.end || new Date().toISOString()
