...
import type { JobShift } from "@/lib/mock-job-shift-data"
...
export function JobDetailsModal({ job, isOpen, onClose }: JobDetailsModalProps) {
  ...
  const handleApply = () => {
    ...
    console.log("Applying for job:", job.id, "with tasks:", selectedTasks)
    onClose()
  }
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {job.assignees.map((assignee) => (
                  <div key={assignee} className="flex items-center gap-2 bg-gray-50 rounded-full px-3 py-1">
